...
def handle_list_result_types(args):
    ...
    def formatted_guidelines(guideline_rules):
        return ";  ".join(
            f"{guideline_rule['type']}: {', '.join(guideline_rule['rules'])}"
    ...
    checker_coverage_stat = [
        {
            ...
            "enabledInAllRuns": True if len(stat.disabled) == 0 else False,
    ...
    if args.output_format == 'json':
        ...
        header = [ "Checker Name", "guideline", "Severity",
                  ...
                  "Outstanding Reports",]
        # header = list(checker_coverage_stat[0].keys()) \
        #     if len(checker_coverage_stat) else []

        # rows = [[str(value) for value in stat.values()]
        #         for stat in checker_coverage_stat]
        rows = [(stat["checker"],
